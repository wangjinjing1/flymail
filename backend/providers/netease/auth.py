...
class NeteaseAuthProvider(AuthProvider):
    """网易邮箱认证提供者（使用授权码认证，不需要OAuth）"""

    def get_auth_url(self, redirect_uri: str = "", state: str = "") -> str:
        """网易邮箱不需要OAuth授权URL，返回空字符串"""
        return ""

    async def handle_callback(self, code: str, redirect_uri: str = "") -> Credentials:
        """网易邮箱不需要OAuth回调，返回空凭据"""
        return Credentials(provider_type="netease")
    ...
    def create_credentials(email: str, auth_code: str) -> Credentials:
        ...
        return Credentials(
            ...
            access_token=auth_code,  # 网易邮箱使用授权码作为访问令牌
            refresh_token="",
            expires_at=0,  # 授权码不过期
            extra={"email": email}
        )
